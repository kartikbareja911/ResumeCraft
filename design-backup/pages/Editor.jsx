...
import { 
  ...
  Sliders, User, Briefcase, GraduationCap, Award, Sparkles, Check, Download, FileText
} from 'lucide-react';
...
const getFontFamilyCSS = (fontName) => {
  if (!fontName || fontName === 'same-as-body') return 'Inter, sans-serif';
...
export default function Editor() {
    ...
    setSections(prev => prev.map(sec => {
      if (sec.id === sectionId) {
        ...
        if (sectionId === 'education') {
          ...
          newItem = { id: Date.now().toString(), name: '', institution: '', startDate: '' };
        } else {
  ...
  const renderSection = (sectionId) => {
    ...
    return (
      <div key={section.id} className="space-y-2 text-[1em]" style={{ marginTop: styles.sectionSpacing }}>
        <h3 
          className="text-[0.9em] font-bold uppercase tracking-wider border-b pb-1"
          style={{ color: styles.primaryColor, borderColor: styles.primaryColor + '30' }}
        >
          {section.name}
        </h3>

        {section.id === 'summary' && section.text && (
          <p className="text-[0.85em] text-slate-600 leading-relaxed text-justify">
            {section.text}
          </p>
        )}
            ...
            {section.items?.map((item) => (
              ...
              >
                <div className="flex justify-between items-baseline">
                  <span className="text-[0.9em] font-bold text-slate-700">
                    {item.position || 'Position Title'} <span className="font-normal text-slate-400">at</span> {item.company || 'Company Name'}
                  </span>
                  <span className="text-[0.8em] text-slate-500 font-medium">
                    {item.startDate || 'Start Date'} – {item.endDate || 'End Date'}
                  </span>
                </div>
                {item.description && (
                  <p className="text-[0.8em] text-slate-600 leading-relaxed whitespace-pre-line text-justify pl-1">
                    {item.description}
                  </p>
                )}
              </div>
          ...
          <div className="space-y-3" style={{ gap: styles.itemSpacing }}>
            {section.items?.map((item) => (
              ...
              >
                <div className="flex justify-between items-baseline">
                  <span className="text-[0.9em] font-bold text-slate-700">
                    {item.degree || 'Degree and Field'}
                  </span>
                  <span className="text-[0.8em] text-slate-500 font-medium">
                    {item.startDate || 'Start Date'} – {item.endDate || 'End Date'}
                  </span>
                </div>
                <div className="flex justify-between text-[0.8em] text-slate-500 font-medium">
                  <span>{item.institution || 'University Name'}</span>
                </div>
                {item.description && (
                  <p className="text-[0.8em] text-slate-600 leading-relaxed whitespace-pre-line pl-1 mt-1">
                    {item.description}
                  </p>
                )}
              </div>
              ...
              >
                <div className="flex justify-between items-baseline">
                  <span className="text-[0.9em] font-bold text-slate-700">
                    ...
                    {item.technologies && <span className="text-[0.85em] font-normal text-slate-500"> ({item.technologies})</span>}
                  </span>
                  <span className="text-[0.8em] text-slate-500 font-medium">
                    {item.startDate || 'Date'}
                  </span>
                ...
                {item.link && (
                  <a href={`https://${item.link}`} target="_blank" rel="noreferrer" className="text-[0.8em] text-indigo-600 hover:underline block font-mono pl-1">
                    {item.link}
                  </a>
                )}
                {item.description && (
        ...
        {section.id === 'skills' && (
          <div className="grid grid-cols-1 gap-1 text-[0.8em]">
            {section.items?.map((item) => (
              <div key={item.id} className="flex items-start">
                <span className="font-bold text-slate-700 w-32 flex-shrink-0">
        ...
        {section.id === 'certificates' && (
          <div className="space-y-1.5" style={{ gap: styles.itemSpacing }}>
            {section.items?.map((item) => (
              <div 
                key={item.id} 
                className="space-y-0.5"
              ...
              >
                <div className="flex justify-between items-baseline">
                  <span className="text-[0.9em] font-bold text-slate-700">
                    {item.name || 'Certificate Title'} <span className="font-normal text-slate-400">by</span> {item.institution || 'Issuing Body'}
                  </span>
                  <span className="text-[0.8em] text-slate-500 font-medium">
                    {item.startDate || 'Date'}
                  </span>
                ...
                {item.description && (
                  <p className="text-[0.8em] text-slate-600 leading-relaxed whitespace-pre-line pl-1">
                    {item.description}
                  </p>
                )}
              </div>
    ...
    <div className={`${editorShellClass} fixed inset-0 flex flex-col overflow-hidden`}>
      ...
      <div className="flex-1 flex overflow-hidden">
                          ...
                          <div className="space-y-4">
                            {section.items?.map((item) => (
                              <div key={item.id} className={itemCardClass}>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  <div>
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => updateListItem(section.id, item.id, 'startDate', e.target.value)}
                                      className={inputClass}
                                  ...
                                  <div>
                                    ...
                                    <input
                                      type="text"
                                      value={item.endDate || ''}
                                      onChange={(e) => updateListItem(section.id, item.id, 'endDate', e.target.value)}
                                      className={inputClass}
                                      placeholder="Present"
                                    />
                                  </div>
                                ...
                                <div>
                                  <label className={labelClass}>Details (Bullet points)</label>
                                  <textarea
                                    rows="3"
        ...
        <main className="flex-1 overflow-auto bg-slate-100 p-8 flex justify-center items-start relative select-text">
          {/* Scaled A4 wrapper reserves the exact visual size, preventing blank scroll space. */}
          <div
          ...
          >
            ...
            >
              {/* A4 Content Wrapper */}
              <div className="w-full flex flex-col h-full text-slate-800">
              
              {/* RENDER LAYOUT TEMPLATE 1: CLASSIC CLEAR (DEFAULT) */}
              {(styles.template === 'classic' || !styles.template) && (
                <div className="w-full flex flex-col h-full">
                  {/* HEADER: Personal Information */}
                  <div className="text-center border-b pb-4 mb-4" style={{ borderColor: styles.primaryColor + '30' }}>
                    <h1 className="text-[2.2em] font-extrabold tracking-tight uppercase" style={{ color: styles.primaryColor, fontFamily: 'var(--name-font-family)' }}>
                      {personalInfo.name || 'Your Full Name'}
                    </h1>
                    <h2 className="text-[1em] font-semibold tracking-wider text-slate-500 uppercase mt-0.5">
                      {personalInfo.title || 'Target Job Title'}
                    </h2>
                    <div className="flex flex-wrap items-center justify-center gap-y-1.5 gap-x-3 text-[0.85em] mt-3 text-slate-600">
                      ...
                      {personalInfo.website && <><span className="text-slate-300">•</span><a href={`https://${personalInfo.website}`} target="_blank" rel="noreferrer" className="hover:underline">{personalInfo.website}</a></>}
                    </div>
                    <div className="flex flex-wrap items-center justify-center gap-x-3 text-[0.8em] mt-1 text-slate-500 font-mono">
                      ...
                      {personalInfo.linkedin && <>{personalInfo.github && <span className="text-slate-300">•</span>}<a href={`https://${personalInfo.linkedin}`} target="_blank" rel="noreferrer" className="hover:underline">{personalInfo.linkedin}</a></>}
                    </div>
                  </div>
              ...
              {styles.template === 'sidebar-left' && (
                <div className="w-full flex h-full flex-1 gap-6 text-slate-800">
                  {/* Sidebar Left Column */}
                  <div 
                  ...
                  {/* Main Right Column */}
                  <div className="flex-1 flex flex-col gap-4 py-1.5 pl-2">
                    {renderSection('summary')}
                    {renderSection('experience')}
                    {renderSection('education')}
                    {renderSection('projects')}
              ...
              {styles.template === 'header-accent' && (
                <div className="w-full flex flex-col h-full text-slate-800">
                  ...
                  <div className="flex justify-between items-start border-b pb-4 mb-4" style={{ borderColor: styles.primaryColor + '30' }}>
                    <div>
                      <h1 className="text-[2.2em] font-extrabold tracking-tight uppercase" style={{ color: styles.primaryColor, fontFamily: 'var(--name-font-family)' }}>
                        {personalInfo.name || 'Your Full Name'}
                      </h1>
                    ...
                    <div className="text-right text-[0.8em] text-slate-600 space-y-1">
                      ...
                      {(personalInfo.github || personalInfo.linkedin) && (
                        <div className="flex gap-2 justify-end font-mono text-[0.95em] text-slate-500">
              ...
              {styles.template === 'sidebar-right' && (
                <div className="w-full flex flex-col h-full text-slate-800">
                  ...
                  <div className="text-center border-t-2 border-b-2 py-4 mb-4" style={{ borderColor: styles.primaryColor }}>
                    <h1 className="text-[2.2em] font-extrabold tracking-wide uppercase" style={{ color: styles.primaryColor, fontFamily: 'var(--name-font-family)' }}>
                      {personalInfo.name || 'Your Full Name'}
                    </h1>
                    <h2 className="text-[1em] font-semibold tracking-wider text-slate-500 uppercase mt-0.5">
                      {personalInfo.title || 'Target Job Title'}
                    </h2>
                    <div className="flex flex-wrap items-center justify-center gap-y-1.5 gap-x-4 text-[0.8em] mt-3 text-slate-600">
                      {personalInfo.email && <span>{personalInfo.email}</span>}
